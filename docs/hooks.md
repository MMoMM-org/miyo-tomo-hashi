...
`<action>` is one of the [action kinds](action-reference.md): `create_moc`, `move_note`, `link_to_moc`, `insert_under_marker`, `replace_section`, `add_relationship`, `edit_note_text`, `remove_up_link`, `update_tracker`, `update_log_entry`, `update_log_link`, `delete_source`, `skip`.
