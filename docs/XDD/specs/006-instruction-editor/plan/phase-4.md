...
- [x] **T4.1 "Open instruction fixer" command** `[activity: frontend-ui]` `[parallel: true]`
  ...
  - Success:
    - [x] Command opens the Fixer; Execute untouched `[ref: PRD/F1-AC1,AC4]` `[ref: SDD/ADR-3]`

- [x] **T4.2 Execute-result surface "Open Instruction Fixer" option** `[activity: frontend-ui]` `[parallel: true]`
  ...
  4. Validate: unit tests pass; lint clean; types check.
  - Success:
    - [x] Failed run offers "Open Instruction Fixer" instead of only the log `[ref: PRD/F1-AC2]` `[ref: SDD/ADR-3]`

- [x] **T4.3 Run-log `I##` deep-link removal + Fixer pointer line** `[activity: backend-api]` `[parallel: true]`
  ...
  - Success:
    - [x] `I##` cell is plain text; pointer line present; log otherwise stable `[ref: PRD/F2-AC4]` `[ref: PRD/F1-AC3]` `[ref: SDD/ADR-8]`
