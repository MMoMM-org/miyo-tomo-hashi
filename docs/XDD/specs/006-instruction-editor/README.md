...
| 2026-07-27 | **ADR-5 amendment: anchor + marker pickers; the write surface widens to the anchor triple** (user, manual QA of `2026-07-25_0947_instructions.json`) | Three findings. **(a)** Card intent lines named only the target, never the payload — I07 read `Link into @, after heading "Maintenance"` with no way to tell WHICH note was being linked. `link_to_moc`/`insert_under_marker`/`replace_section` now name the text they write, taken from the field the executor writes (`line_to_add`/`content`), never from `source_note_title` (Tomo may emit it null, and a card that showed a label while the executor wrote something else would mislead exactly the user about to repair the action). **(b + c)** The **Anchor** and **Marker** fields gain a `Choose…` button sourced from the TARGET note, read fresh from content at click time (#68 rule, never the metadataCache). **The amendment:** one anchor pick commits `anchor.type` + `anchor.value` + `placement` **together** — ADR-5 originally made only `anchor.value` writable, which a picker cannot honour: choosing a callout to replace a renamed heading changes the type as a direct consequence, and writing one third of the triple emits an anchor the resolver cannot resolve. `TARGET_FIELD_WHITELIST` is unchanged and still governs every free-text field. **No schema change** — `type` and `placement` are existing enum fields (ADR-6 intact). Placement legality is **kind-aware**, mirroring each handler: `link_to_moc` rejects `inside` on anything but a callout, `insert_under_marker` additionally supports `inside` on a heading, `replace_section` takes headings only with no placement; `suggestions/validPlacements` encodes only the first, so reusing it would have silently dropped the second. `block` anchors are never constructed (no principled enumeration; field stays free text). New `src/instruction-fixer/noteSpots.ts` (pure) + `src/ui/instruction-fixer/pickers/`; callout enumeration moved into `markdownStructure` and spec-004's `SpotPicker` dropped its local copy. Both rosters are **cross-checked against the executor** in tests (every anchor through `resolveAnchor`, every marker through the real `addRelationship`), and both cross-checks were mutation-verified. Commit `b215fe7`; 141 test files / 2352 tests green. |

| 2026-07-27 | **Marker picker seeds `line` too** (user, manual QA of the picker just shipped above) — **⚠️ reverted same day, see next row** | Confirmed the danger the previous row's design left open: picking a new `marker` while an unrelated `line` sat untouched let `add_relationship` silently overwrite a line that had nothing to do with the one just picked — reproduced live on I08 (`marker` repointed to an arbitrary body line, `line` still `"up:: [[@]]"`, which would have clobbered that line on re-run). `MarkerSpot` gains a `line` field (prefix-stripped, same structural stripping the handler re-applies on write) and the card's marker pick now writes `marker` + `line` in one transform. Cross-checked: writing a spot's own seeded `line` unmodified round-trips through the real `addRelationship` handler as `skipped-already`, not a rewrite — mutation-verified. No further schema/whitelist change; `line` was already writable per ADR-5. |
| 2026-07-27 | **Correction: revert the `line` seed — `marker` and `line` are independent ON PURPOSE** (user) | The previous row's fix solved the wrong problem. `marker` says WHERE to write; `line` says WHAT relationship to establish there. The usual reason to pick a NEW marker is repositioning to a better anchor — e.g. a template placeholder like *"No parent map yet — this note is floating."* rather than an existing `up::` field — which must leave the relationship being written (`line`) untouched. Seeding `line` from the picked spot's own current text instead made every fresh placement a no-op: the handler would find the placeholder and "replace" it with the placeholder text itself, establishing nothing. Reverted `MarkerSpot.line` and the double `setTargetField` write; the marker pick is back to a single-field commit. `detail` (the current line, shown in the picker row) stays display-only — it is the actual safeguard (see what you're about to overwrite before picking), not a silent auto-fill. |
| 2026-07-27 | **Second correction: `setMarkerSpot` swaps the `line` PREFIX, keeps the suffix** (user, I08's actual failure — `marker: "up::"` doesn't match anything in the target note) | The revert above was ALSO incomplete — leaving `line` fully untouched can't fix I08's real complaint: the stale `up::` text embedded at the front of `line` never tracked the newly picked marker. New model: when `line` CURRENTLY starts with the CURRENT `marker`, picking a new marker rewrites only that prefix — `up:: [[@]]` → `parent:: [[@]]` for a pick of `parent::` — preserving everything after it verbatim (the actual link payload). When `line` does NOT start with `marker` (Tomo's own multi-link aggregation can produce that state), there is no reliable split point, so only `marker` changes — the same safe fallback the previous revert used unconditionally, now scoped to the one case it's actually needed for. New `setMarkerSpot` transform in `transforms.ts`; `MarkerSpot` itself carries no `line` field, `detail` stays display-only. |
