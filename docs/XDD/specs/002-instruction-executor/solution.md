...
CON-7 **Bundle budget**: informal target ≤ 980 KB (raised 2026-08-31 for xterm 6) total `main.js`, inherited from 001's revised CON-7 (the original 500 KB target was set before 001 discovered dockerode + docker-modem must be bundled rather than externalized; combined ~940 KB pre-002). Wiring the 002 surfaces (executor + planner + 8 handlers + run log + hook runner + 3 UI surfaces + ajv runtime) brought the post-002 build to ~1105 KB, and the ceiling was mechanically raised to 1200 KB at T6.2. The #46 bundle audit then found dockerode 5.x EAGERLY requires its `./session` gRPC BuildKit helper (`@grpc/grpc-js` + `@grpc/proto-loader` + `protobufjs`, ~1 MB pre-minify / ~414 KB minified) — dead code in Hashi, which never builds images. Stubbing `./session` at build time (alongside the existing `ssh2`/`cpu-features`/`./buildkit` stubs in `esbuild.config.mjs`) dropped the bundle to ~759 KB; the ceiling was lowered to 850 KB (~12% headroom). Two raises followed: spec-006 T3.1 (2026-07-26) wired the Instruction Fixer into `main.ts`, making previously tree-shaken Phase 1-2 code reachable (~848 → ~860 KB, ceiling → 900 KB); and the xterm 5.5.0 → 6.0.0 bump (2026-08-31), which integrates VS Code's `base/` platform layer and its own scroll bar (882.3 → 938.0 KB, ceiling → 980 KB — the first raise driven by a dependency rather than by Hashi's own surface area). This stays under spec-001's CON-7 ceiling of 1000 KB. Remaining levers if it climbs again: ajv standalone code-gen (ADR-1 v1) and lazy-loading xterm.js (only 001's TomoChat needs it, not the executor).
