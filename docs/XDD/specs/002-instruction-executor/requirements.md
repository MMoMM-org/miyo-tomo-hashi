...
- **Acceptance Criteria:**
  ...
  - [ ] `link_to_moc`: Given the target MOC exists (on disk or created earlier in this run) and the action's `anchor` resolves in the MOC, Then `line_to_add` is inserted at the position determined by `placement`. The action carries `anchor: {type: "callout"|"heading"|"line", value: string}` and `placement: "inside"|"after"`. **Anchor matchers** (Hashi-side): `callout` matches a callout opening line by `[!type] Title` (case-insensitive); `heading` matches a heading by text without `#`-prefix (case-sensitive, any level); `line` matches the first body line whose stripped content contains the value verbatim (substring). **Placement** (Hashi-side): `inside` is callout-only — Hashi prepends `> ` to `line_to_add` and inserts as the last line of the callout body; `after` inserts verbatim immediately after the anchor's terminal line (callout: after the closing `>` line; heading: after the heading line itself, NOT after section content; line: after the matched body line). `placement: "inside"` on a non-callout anchor fails with *"placement: inside requires callout anchor"*. `line_to_add` is **pre-formatted by Tomo** (bullet style, link form); Hashi writes it verbatim, prepending `> ` only for inside-callout. **No fallback** to "first editable callout" — anchor-not-found is a hard fail. Given the identical line already in the MOC at the anchor target, Then no-op. Given the MOC is unreachable, Then fail *"MOC target missing"*. Given `anchor.value` is null, Then fail *"anchor not found (null value)"*.
