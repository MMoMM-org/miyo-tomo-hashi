...
> **AC count gate**: `grep -c '^  - \[ \]' requirements.md` is the canonical AC total. Plan T6.5 reads it at run time. Last counted: 99 ACs (2026-06-23, +1 insert_under_marker).
...
#### F4: Execute the 9 Action Kinds in Canonical Order
- **User Story:** As the PKM Author, I want the executor to perform each of the 9 action kinds described in Tomo's consumer contract, in the canonical order, against the absolute paths Tomo emits, so that applied edits match the reviewed plan exactly.
- **Acceptance Criteria:**
  - [ ] Within each `_instructions.json`, the executor processes actions in canonical order: `create_moc` → `move_note` → `link_to_moc` → `insert_under_marker` → `add_relationship` → `update_tracker` → `update_log_entry` → `update_log_link` → `delete_source` → `skip`. Within each kind-block, actions execute in monotonic `I##` order.
  ...
  - [ ] `link_to_moc`: Given the target MOC exists (on disk or created earlier in this run) and the action's `anchor` resolves in the MOC, Then `line_to_add` is inserted at the position determined by `placement`. The action carries `anchor: {type: "callout"|"heading"|"line", value: string}` and `placement: "inside"|"before"|"after"`. **Anchor matchers** (Hashi-side): `callout` matches a callout opening line by `[!type] Title` (case-insensitive); `heading` matches a heading by text without `#`-prefix (case-sensitive, any level); `line` matches the first body line whose stripped content contains the value verbatim (substring). **Placement** (Hashi-side): `inside` is callout-only — Hashi prepends `> ` to each line of `line_to_add` and inserts as the last line(s) of the callout body; `before` inserts verbatim immediately before the anchor's first line (callout opener / heading line / matched line); `after` inserts verbatim immediately after the anchor's terminal line (callout: after the closing `>` line; heading: after the heading line itself, NOT after section content; line: after the matched body line). `placement: "inside"` on a non-callout anchor fails with *"placement: inside requires callout anchor"*. **Multi-line `line_to_add`** (added 2026-06-13): `line_to_add` MAY contain embedded `\n`; every line is written as a block with blank lines preserved — verbatim for `before`/`after`, each line `> `-prefixed for `inside`. `line_to_add` is **pre-formatted by Tomo** (bullet style, link form); Hashi writes it verbatim, prepending `> ` only for inside-callout. This is Hashi's single general insert primitive `(anchor, placement)` — Tomo decides the text and position; no per-feature apply verb. **No fallback** to "first editable callout" — anchor-not-found is a hard fail. Given the identical block already present in the MOC (consecutive lines), Then no-op. Given the MOC is unreachable, Then fail *"MOC target missing"*. Given `anchor.value` is null, Then fail *"anchor not found (null value)"*.
  - [ ] `insert_under_marker`: Given the target note at `target_path` exists and the action's `anchor` resolves, Then `content` is inserted at the position determined by `placement`. Generalises `link_to_moc`'s insert primitive to an **arbitrary** note (not a MOC stem) with a **multi-line** `content` payload. The action carries `target_path: string` (vault-relative path to any existing note), `anchor: {type: "callout"|"heading"|"line", value: string}` (same matchers as `link_to_moc`), `placement: "inside"|"before"|"after"`, and `content: string` (multi-line markdown block). **Placement** (Hashi-side): `inside` on a **callout** appends `content` (each line `> `-prefixed) as the last line(s) of the callout body; `inside` on a **heading** appends `content` verbatim at the **end of the heading's section** — immediately above the next heading of same-or-higher level, or EOF (reuses the `update_log_entry` section locator); `inside` on a **line** anchor fails with *"placement: inside not supported for line anchor"*; `before`/`after` insert `content` verbatim relative to the marker (any anchor type), same as `link_to_moc`. **Modify-only** — Tomo guarantees `target_path` exists before emitting; a missing note fails *"target note missing"* (never created). **Append, never replace.** Given `anchor.value` is null, Then fail *"anchor not found (null value)"*. Given the anchor cannot be resolved, Then fail *"anchor not found: <value>"* with no mutation. Given the identical block is already present (consecutive lines), Then no-op (`skipped-already`). `schema_version` stays `"1"` — purely additive. (Added 2026-06-23 for Tomo spec 024 tag-handler framework, `miyo-tomo#47`.)
  - [ ] `add_relationship`: Given the target MOC exists, Then the first line whose stripped content (after optional `> ` callout prefix and whitespace) starts with `marker` is replaced by `line` verbatim — Hashi normalises the callout prefix to canonical `> ` (single space) when the matched line was inside a callout, no prefix otherwise. The action carries `target_moc_path: string`, `marker: string` (e.g., `"up::"`, `"related::"`), and `line: string` (full Dataview-style line e.g., `"up:: [[Hobbies (MOC)]]"`). Multi-link aggregation for `related::` is done **Tomo-side** before emission — Hashi only writes `line` verbatim. Given the located line already equals the target `line` (with prefix normalised), Then no-op. Given the MOC is unreachable, Then fail *"Relationship target missing"*. Given no line in the MOC starts with `marker`, Then fail *"Marker not found: <marker>"*.
  - [ ] `update_tracker`: Given the target file exists and the tracker field is reachable in the specified mode (`inline_field` / `callout_body` / `checkbox`), Then the field is set to the target value. Given it already holds that value, Then no-op. Given a different value, Then **overwrite the existing value with the target value** (Tomo's intent wins — the upstream review step is the approval gate; Hashi's role at execute time is to apply, not to re-arbitrate). The `inline_field` matcher recognises three Dataview positions: line-anchored (`<field>:: …`, with optional bullet/whitespace prefix), inline-bracketed (`[<field>:: …]` anywhere on the line), and inline-parenthesized (`(<field>:: …)` anywhere on the line); the matched form is preserved byte-for-byte on overwrite — only the value portion is rewritten. Insertion of a new field, when needed, uses the line-anchored form only. The `callout_body` syntax requires double-colon (`> <field>:: …`); single-colon is not a supported variant. Multi-word field names (`For Me`, `Learned Words`) are matched literally.
  - [ ] `update_log_entry`: Given the target file exists, Then a prose line is inserted at the specified position. The Hashi-composed line shape is: `- <content>` for `after_last_line` / `before_first_line`; `- HH:MM: <content>` for `at_time`. Tomo emits `content` as bare prose (no bullet, no time prefix); Hashi composes the bullet and time prefix. Given identical line at that position, Then no-op.
  - [ ] `update_log_link`: Given the target file exists, Then a wikilink line is inserted at the specified position. The Hashi-composed line shape is: `- [[<target_stem>]]` for `after_last_line` / `before_first_line`; `- HH:MM: [[<target_stem>]]` for `at_time`. The two log-targeting kinds (`update_log_entry` + `update_log_link`) share the same time-prefix shape since they coexist in the same Daily Log section. Given identical line, Then no-op.
  - [ ] `delete_source`: Given source exists, Then it is deleted via Obsidian's `FileManager.trashFile`, honoring the user's "Files & Links → Deleted files" preference (system trash / vault-local `.trash/` / **permanent**). **Amended 2026-06-12** (Kokoro decision; supersedes the ratified "never hard-deleted" guarantee): the obsidianmd `prefer-file-manager-trash-file` lint rule and respect for the user's explicit deletion preference take precedence over forcing recoverable deletion. **Safety net**: when that preference is "Permanently delete", Hashi surfaces a **one-time** warning (a `Notice`, not a per-execution prompt) that `delete_source` will permanently, non-recoverably delete the source file. Given source already gone, Then no-op. **`source_path` contract** (per Tomo `docs/instructions-json.md` § Path Shape Contract, 2026-04-30): the path is byte-equal to the matching `.md` peer's wikilink target — Hashi consumes verbatim, never appending/stripping extensions. **Peer-independence**: media + transcript files that share a stem (e.g., `Voice…m4a` + `Voice…m4a.md`) are emitted as separate actions reflecting separate user decisions in Tomo's review step — Hashi MUST NOT infer peer deletion from shared stems, filename prefixes, or any other heuristic. The user CAN delete the audio while keeping the transcript (or vice versa); the action list is the authoritative intent. **Forbidden-character files** (e.g., recorder-named `.m4a` containing `:` on macOS) appear in actions with their literal on-disk names — Hashi does NOT reverse-sanitise; Tomo's `sanitize_stem` is applied only to files Tomo creates.
