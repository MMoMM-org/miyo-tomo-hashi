...
| F4.delete_source.trash | `delete_source`: source is deleted via `FileManager.trashFile`, honoring the user's "Deleted files" preference (system trash / `.trash/` / permanent). With pref = system or `.trash`, the file is recoverable from that location (F4 amendment 2026-06-12) | | | |
| F4.delete_source.permanent-warning | With Obsidian "Files & Links → Deleted files" = "Permanently delete": first run containing a `delete_source` shows a one-time Notice that deletion is permanent/non-recoverable; the source is hard-deleted. Subsequent runs do NOT re-show the Notice (one-shot flag persisted) | | | |
| F4.delete_source.idempotent | Re-run when source already gone: no-op | | | |
| F4.delete_source.peer-independent | Audio + transcript scenario (test/Hashi/100 Inbox I31/I32/I36/I39 pattern): four separate actions covering audio-1 delete + transcript-1 delete + audio-2 delete + transcript-2 skip. Verify each acts only on its own `source_path` — no peer inference, no shared-stem heuristic. Audio-2 trashed AND transcript-2 retained must both be true after the run. | | | |
| F4.delete_source.colon-bearing-media | `delete_source` on a `.m4a` file whose name contains `:` (recorder-emitted forbidden char on macOS): expected outcome is either `applied` (Obsidian's vault adapter reaches the file) OR `skipped-already` (adapter cannot resolve the literal path). **Either way the run continues** — no abort, no error glyph. If `skipped-already` is the consistent result, surface in run log + post-walk follow-up to Kado / hook author. | | | |
