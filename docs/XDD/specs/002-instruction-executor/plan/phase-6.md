...
- [ ] **T6.2 main.ts wire-up** `[activity: integration]`
  ...
  2. Test: `test/unit/main.test.ts` — simulate plugin lifecycle:
     - `onload`: 002 wiring instantiates `ObsidianVaultFS`, `SchemaValidator` (importing `validator.gen.js`), `HookRunner`, `InstructionExecutor`, `ExecutionModal` (modal class only — instance created on demand), `StatusBar`, `SettingsTab` (extension, not replacement), `registerCommands`, `registerEvent('file-menu')`
...
- [ ] **T6.3 End-to-end live test — fixtures + executor.live.test.ts** `[activity: testing]`
  ...
  2. Test: Build out `test/live/executor.live.test.ts` using `FsPromisesVaultFS` against `os.tmpdir()`. Each scenario seeds a temp vault, runs `InstructionExecutor.execute()`, and asserts vault state + run-log contents.
...
- [ ] **T6.5 PRD AC traceability table + spec finalization + Phase 6 Validation** `[activity: validate]`
  ...
  1. Prime: Read PRD §Feature Requirements (F1–F11) — count ACs `[ref: PRD/Feature Requirements]`. Expected total: 95 (F1=9, F2=7, F3=9, F4=15, F5=8, F6=5, F7=11, F8=13, F9=6, F10=9, F11=4).
  2. Test: Build a traceability matrix file `docs/XDD/specs/002-instruction-executor/plan/traceability.md`:
     - Rows: every PRD AC (95 total)
  ...
  5. Success:
     - [ ] 95/95 ACs traced to at least one test/live/manual artifact `[ref: PRD/Feature Requirements]`
