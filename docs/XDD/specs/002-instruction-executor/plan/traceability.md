...
| F4.6 | `link_to_moc`: anchor (callout/heading/line) × placement (inside/after); pre-formatted line; idempotent; missing MOC / missing anchor / inside-non-callout all fail; no fallback | `test/unit/actions/linkToMoc.test.ts`, `test/unit/actions/anchorResolver.test.ts` | F4.link_to_moc.callout-inside, F4.link_to_moc.callout-after, F4.link_to_moc.heading-after, F4.link_to_moc.line-after, F4.link_to_moc.duplicate, F4.link_to_moc.anchor-missing, F4.link_to_moc.inside-non-callout, F4.link_to_moc.missing-moc | ✅ |
