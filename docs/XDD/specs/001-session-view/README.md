...
| 2026-05-02 | **Multi-perspective review pass on shipped spec-001 (review/spec-001-fixes branch).** Seven specialist reviews (Security, Robustness, Performance, Quality, Accessibility, Compatibility, Testing) ran against the spec-001 surface as it stands on `main` — diff `3c92f66..fda801b`. 1 Critical, 6 High, 18 Medium, 19 Low findings synthesised. C1 chat-view focus-on-open + ItemView focus override (keyboard users couldn't type without clicking). H1 PRIVACY.md authored as Constitution L1 release-blocker (network surfaces, persisted data, hooks ctx scope, "what Hashi does not do"). H2 stale FS2 assertion in live e2e test (`expect(...).toBe(c.id)` → `.toBe("e2e-test")`). H3 dialAttach unit test added via local TCP http.Server (review/spec-001 H3 — H4 closed by design: live tests are intentionally manual, mirrors spec-002 L12 pattern). H5/H6 zoom buttons aria-pressed + picker aria-live. M1 Kokoro handoff filed (`_outbox/for-kokoro/2026-05-02_hashi-to-kokoro_label-contract-delta-spec-001.md`) for instance-name label addition + plugin-enabled v0.1 decision. M2 dialAttach 10s upgrade timeout. M3 non-TTY PassThrough cleanup. M5 persistChosenInstanceBestEffort helper — persist failures no longer roll back the live AttachSession. **M6 image-verification accepted-as-out-of-scope for v0.1**: discovery filters by `miyo.component=tomo` label only; any container self-stamping that label is attachable. Image allow-list would require a Tomo-side commitment about predictable image identifiers; cross-repo concern, deferred to v0.2 with an `expected_image` setting. **M7 Notice on auto-attach from saved session**: Obsidian Sync propagates `data.json` across devices; auto-reconnect on a second device could land on a same-name container that's not the one the user originally chose. main.ts now fires `Notice("Auto-connecting to '<name>' from saved session")` before `autoReconnectIfRemembered()` so a wrong-container attach is detectable rather than silent. | Image-verification pinning is the kind of cross-repo assertion that needs a Kokoro ADR before becoming a Hashi setting — recorded here as a v0.1 boundary so the next reviewer doesn't re-raise it. |
