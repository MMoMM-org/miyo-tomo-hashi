...
CON-11 **No per-OS runtime gating**: v0.1 performs **no** per-OS detection, branching, or warning beyond the binary `manifest.isDesktopOnly: true`. The charter tiers (macOS primary / Linux experimental / Windows user-contributed) are **support expectations**, not code-enforced behavior — Hashi loads and runs identically on every desktop OS, and Docker reachability is the only runtime gate (failures surface as the named errors `daemon-unreachable` / `socket-permission-denied` regardless of OS). A load-time per-OS Notice was considered and rejected for v0.1 (ceremony: macOS — the primary platform and most users — would see nothing, while Linux/Windows users would get a "scary banner" on a plugin that may work fine; the named Docker errors already communicate any real failure). The sole platform branch in code is functional, not a gate: `dialAttach` selects the Windows named pipe vs the POSIX socket path. Revisit if real per-OS divergence emerges post-v0.1.
...
**OS support tiers** (macOS primary / Linux experimental / Windows user-contributed, per ADR-009 charter) are **support expectations, not code-enforced** — Hashi runs identically on every desktop OS with no per-OS gating beyond `isDesktopOnly`. See CON-11 for the rationale (a load-time per-OS Notice was considered and rejected for v0.1).
