...
| **Open suggestions editor** | Opens the [Suggestions Editor](suggestions-editor.md) for a Tomo `_suggestions.json` review surface. With a `_suggestions.json` (or its `.md` peer, incl. the `-fan` Force-Atomic Resolve variant) active, opens that run; otherwise offers a picker over every `*_suggestions.json` in the vault. If none exist, shows *"Open a Tomo _suggestions.json (or its .md) first"*. | None — behaviour depends on the active file. | None |
