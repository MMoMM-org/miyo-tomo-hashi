...
| **Open instruction fixer** | Opens the [Instruction Fixer](instruction-fixer.md) — a repair bench for actions that failed or were skipped on a previous `/execute` run. With an `_instructions.json` (or its `.md` peer) active, opens that set directly; otherwise opens a picker over every instruction set in the vault. Empty vault shows *"No instruction sets found — open a _instructions.json (or its .md) first"*. | None — behaviour depends on the active file. | None |
